...
def main():
    ...
    while photo_error_val > 500 and step < 5000:
        ...
        print(f12.shape)
        print("Flow calculation time : ", 2*(time.time() -flow_time))
        print("Scene : ", folder)
        if depth_type == 'TRUE':
            vel, Lsx, Lsy = intermat.getData(d1)
        ...
        for cnt in range(ITERS):
            ...
            vs_lstm.zero_grad()
            f_hat = vs_lstm.forward(vel, Lsx, Lsy, f12)
        ...
        ovhT = time.time()
        f_hat = vs_lstm.forward(vel, Lsx, Lsy, f12.to(torch.device('cuda:0')))
