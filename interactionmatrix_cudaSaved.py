...
class InteractionMatrix():
    def getData(self, d1):
        ...
        xyz = np.fromfunction(lambda i, j, k : 0.5*(k-1)*(k-2)*(j-float(Cx))/float(kx)- k*(k-2)*(i-float(Cy))/float(ky) + 0.5*k*(k-1)*
                ((d1[i.astype(int), j.astype(int)]==0)*med + d1[i.astype(int), j.astype(int)]), (d1.shape[0], d1.shape[1], 3), dtype=float)
