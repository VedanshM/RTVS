...
def main():
    ...
    ITERS = 6 # iterations per MPC step 
    SEQ_LEN = 10
    ...
    while photo_error_val > 500 and step < 5000:
        ...
        if depth_type == 'TRUE':
            ...
            if step == 1:
                ...
                pre_img_src = read_gen(folder + "/results/test.rgba.%05d.%05d.png" % (step-2, step-2))
                img_src = read_gen(folder + "/results/test.rgba.%05d.%05d.png" % (step-1, step-1))
                ...
                vel, Lsx, Lsy = intermat.getData(f12, 0.4*(1/(1+np.exp(-1/flow_depth)) - 0.5))
        ...
        if step == 1:
            if vel_init_type == 'RANDOM':
                ...
                vel = torch.tensor(vel, dtype =torch.float32).to(device="cuda:0")
            
            elif vel_init_type == 'IBVS':
                vel = np.random.normal(size=[6])
                vel = torch.tensor(vel, dtype = torch.float32).to(device="cuda:0")
        else:
            vel = torch.tensor(vs_lstm.v_interm[0], dtype = torch.float32).to(device="cuda:0")
        ...
        for cnt in range(ITERS):
            ...
            # early_stopping(loss, vs_lstm)

            # if early_stopping.early_stop:
            #     print("Early stopping")
            #     break

#NQ            if np.sqrt(loss.item()) < 1000:
#                horizon = 6
...
#NQ                    g['lr'] = 0.01
        ...
        '''
        for i in range(5):
            f_interm = flow_to_image(vs_lstm.f_interm[i])
            f_interm = Image.fromarray(f_interm)
            f_interm.save(folder + "/flo/" + "%05d.%05d.png" % (step, i))
        '''
        ...
        #if step == 0 :
        #    torch.nn.init.xavier_normal_(vs_lstm.linear.weight)
        #    torch.nn.init.normal_(vs_lstm.linear.bias, 0, 0.5)
