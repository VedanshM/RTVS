...
from model_bidirectional import VisualServoingLSTM
from interactionmatrix import InteractionMatrix
import habitatenv_bidirectional as hs
import imageio
...
def main():
    ...
    NUM_LSTM_UNITS = int(sys.argv[13])
    LR = 0.0001
    ...
    vs_lstm = VisualServoingLSTM(rnn_type=rnn_type, lstm_units=NUM_LSTM_UNITS, layers=NUM_LAYERS, seq_len=SEQ_LEN).cuda()
    ...
    step=0
    while photo_error_val > 500 and step < 1000:
        ...
        if step == 0:
            ...
            vel = torch.tensor(vs_lstm.v_interm[0][0], dtype = torch.float32).cuda()
        ...
        if depth_type == 'FLOW':
            img_src, pre_img_src, d1 = env.example(vs_lstm.v_interm[0][0], step+1, folder)
        elif depth_type == 'TRUE':
            img_src, d1 = env.example(vs_lstm.v_interm[0][0], step+1, folder)
