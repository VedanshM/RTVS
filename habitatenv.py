...
class HabitatEnv():
    ...
    def agent_controller(self, agent, velocity):
        ...
        if self.noise:
            ...
            velocity[5img = cv2.imread('./Baseline/skokloster-castle/results/test.rgba.00000.00000.png')
plt.imshow(img)] += (t_noise[1] - t_noise[2])
    ...
    def example(self, vel, frame=1,folder=''):
        ...
        vel[:, 5] = 0*vel[:, 5] # conventions Z axis
        vel[:, 4] = 0*vel[:, 4] # conventions Y axis
        vel[:, 3] = 0.01*vel[:, 3] # conventions X axis
        ...
        for i in range(vel.shape[0]):
            state = agent.get_state()
            print(vel[i])
            state.position = state.position + vel[i][0:3]
            r = R.from_euler('xyz', vel[i][3:], degrees=False)
            print(r.as_quat())
            x = state.rotation + state.rotation * np.quaternion(0, vel[i][4], vel[i][3], vel[i][5])
            state.rotation = x
            agent = self._sim.initialize_agent(0, state)
...
if __name__ == '__main__':
    print("In Main")
    env = HabitatEnv('Baseline/skokloster-castle', [-1.047, -0.199, 20.145, 0, 0,0, 0], 'TRUE')
    import cv2
    cv2.imwrite('test.png', env._sim.get_sensor_observations()["color_sensor"])
